...
def test_solve_cle():
    ...
    sol = solve_cle(model,np.linspace(0,1000000,100),0,generator=np.random.default_rng(10))
    assert sol[-1] == np.array([0])

    sol2 = solve_cle(model,np.linspace(0,1000000,100),1,np.random.default_rng(10))
    assert sol[-1] == np.array([0])
...
def test_euler_maruyama():
    ...
    sol = euler_maruyama(np.array([0]),np.linspace(0,100,2),f,G,generator=np.random.default_rng(10))
    assert sol[-1][0] == 100
...
def test_euler_maruyama_2():
    ...
    for i in range(0,100000):
        sols[i] = euler_maruyama(np.array([0]),np.linspace(0,100,2),f,G,np.random.default_rng(10))[-1]
    
    assert abs(np.average(sols)) <= 100
